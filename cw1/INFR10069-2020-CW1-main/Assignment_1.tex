...
\newcommand{\assignmentAuthorName}{s1824891}
%--------------------------------------------------------
...
\begin{answerbox}{10em}
The obtained parameters are [17.89768026, 1.44114091], where the first element represents the intersection of the line with the y-axis which is the exam score, and the second element represents the slope of the regression line created by revision_time and exam_score. 
\end{answerbox}
...
\begin{center}
\includegraphics[width=0.7\textwidth]{results/Q1c.jpg}
\end{center}
\end{answerbox}
...
w0 = (np.sum(y) - m *np.sum(x)) / len(x)
w1 = (len(x) * np.sum(x*y) - np.sum(x) * np.sum(y)) / (len(x)*np.sum(x*x) - np.sum(x) ** 2)
w = [w0,w1]
\end{answerbox}
...
\begin{answerbox}{10em}
\mathit{MSE}=\frac{1}{n} \sum_{i=1}^n (y_{i}-$\hat{y}$_{i})^2 \\[2ex]
...
Mean squared error using numpy: 30.98547261
From the displayed data above, it can be seen that the outputs are similar. The only difference would be that the calculated MSE using numpy is rounded up to 8 decimal places, while the calculated MSE using scikit-learn i rounded up to 14 decimal places.
\end{answerbox}
...
\begin{answerbox}{35em}
\includegraphics[width=0.7\textwidth]{results/Q1g.jpg}
From the plotted graph, the minimum Mean Squared Error is calculated to be 32.48096161535148 with its relevant index in x-axis to be 83.
\end{answerbox}
